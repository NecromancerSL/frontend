...
import { Container, Card, CardHeader, CardContent, Grid, Button, FormControl, InputLabel, Select, MenuItem, TextField, Box, TableContainer, Paper, Table, TableHead, TableRow, TableCell, TableBody, IconButton, Snackbar, Dialog, DialogTitle, DialogContent, DialogActions } from '@mui/material';
...
function ProductDetail() {
  ...
  useEffect(() => {
    api.get<Product>(`/listarproduto/${produtoid}`).then((response) => {
      setProduct(response.data);
    });

    const produtoId = Number(produtoid);

    api.get<Comentario[]>(`/comentarios/${produtoId}`).then(async (response) => {
  ...
  const handleSaveEditedComment = () => {
    // Implemente a lógica para salvar o comentário editado no servidor
    // Atualize a lista de comentários após a edição
    setOpenEditModal(false);
  };

  const handleDeleteComment = (commentId: number) => {
    // Implemente a lógica para excluir o comentário no servidor
                ...
                <TableBody>
                  {comments.map((comment) => (
                    <TableRow key={comment.id}>
                      ...
                      <TableCell>{comment.comentario}</TableCell>
                      <TableCell>
